<#
    Builds SaveBackup.dll — a development tool, not a released mod.

    It copies the vault saves aside when the game starts, so a bad build costs a restart
    rather than progress. There is no release archive: this is never shipped to players.
...
$src     = Join-Path $PSScriptRoot "src\SaveBackupPlugin.cs"
...
if ($Install) {
    ...
    Write-Host "Remove it before shipping with:  .\build.ps1 -Uninstall" -ForegroundColor Yellow
}
